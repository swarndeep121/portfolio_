...
export function Astronaut(props) {
  ...
  const yPosition = useMotionValue(5); // for falling animation
  const ySpring = useSpring(yPosition, { damping: 30 }); // damping gives the smooth falling animation
  useEffect(() => {
    ySpring.set(-1);
  }, [ySpring]);
  useFrame(() => { // useFrame effect from react 3 fiber in  order to apply this falling effect to our 3d model
    group.current.position.y = ySpring.get();
  });
  return (
    <group
      ref={group}
      {...props} // props here for getting values from outside
      dispose={null}
